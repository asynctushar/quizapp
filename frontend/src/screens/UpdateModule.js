...
import { setIsModuleUpdated } from "../redux/slices/testSlice";
import { getModule, updateModule } from "../redux/actions/testAction";
...
const UpdateModule = () => {
	...
	const { isModuleUpdated, isLoading, module } = useSelector(
	...
	}, [id, dispatch, moduleId]);

	useEffect(() => {
		if (module) {
			setName(module.name);
			setStandardTime(module.standardTime / 1000)
		}
	}, [module]);

	useEffect(() => {
		if (isModuleUpdated) {
			navigate(`/admin/test/${module.test._id}/modules`);
			dispatch(setIsModuleUpdated(false));
		}
	}, [isModuleUpdated, dispatch, navigate, module]);
	...
	const handleSubmit = (event) => {
		...
		dispatch(updateModule(formData, moduleId));
	};

	return (
		<Fragment>
			<Meta title="Update Module" />
			<div className="flex">
				<SideBar />
				{isLoading ? (
					<Loader />
