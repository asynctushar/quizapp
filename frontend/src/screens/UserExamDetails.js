...
import { getUserExamDetails } from "../redux/actions/examAction";
...
const UserExamDetails = () => {
	...
	const moduleBasedWrongReasons = moduleBasedWrongAnswers?.map((module) => {
		const wrongReasons = module?.questions?.flatMap((question) => {
			...
			return wrongOptions?.map(
				(option) => option?.reason ?? "Conceptual understanding",
			);
		});
	...
	const moduleBasedMostCommonReason = moduleBasedWrongReasons
		?.map((module) => {
			...
			module?.reasons?.forEach((reason) => {
				reasonCounts[reason.trim()] =
					(reasonCounts[reason.trim()] || 0) + 1;
			});
	...
	function calculateModuleBasedSubjectPercentages(
		moduleBasedWrongSubjects,
		exam,
	) {
		const moduleBasedSubjectCounts = moduleBasedWrongSubjects?.map(
			(module) => {
				const subjectCounts = {};
				...
				const subjectsAndCounts = Object.keys(subjectCounts).map(
					(subject) => [subject, subjectCounts[subject]],
				);
		...
		function calculateSubjectPercentages(
			moduleBasedSubjectCounts,
			moduleBasedTotalSubjectCounts,
		) {
			const subjectPercentages = [];

			for (let i = 0; i < moduleBasedTotalSubjectCounts?.length; i++) {
				const moduleTotalSubjectCount =
					moduleBasedTotalSubjectCounts[i];
				...
				for (const subject in moduleTotalSubjectCount.subjectCounts) {
					if (
						moduleTotalSubjectCount.subjectCounts.hasOwnProperty(
							subject,
						)
					) {
						const totalSubjectCount =
							moduleTotalSubjectCount.subjectCounts[subject];
						const subjectCount = moduleBasedSubjectCounts[i]?.find(
							(entry) => entry[0] === subject,
						);

						if (subjectCount) {
							const [_, count] = subjectCount;
							const percentage = Math.round(
								(1 - count / totalSubjectCount) * 100,
							);
		...
		const moduleBasedSubjectPercentages = calculateSubjectPercentages(
			moduleBasedSubjectCounts,
			moduleBasedTotalSubjectCounts,
		);
	...
	// Usage
	const moduleBasedSubjectPercentages =
		calculateModuleBasedSubjectPercentages(moduleBasedWrongSubjects, exam);

	console.log(moduleBasedSubjectPercentages);

	function calculateModuleBasedSkillPercentages(
		moduleBasedWrongSkills,
		exam,
	) {
		const moduleBasedSkillCounts = moduleBasedWrongSkills?.map((module) => {
			...
			const skillsAndCounts = Object.keys(SkillCounts).map((Skill) => [
				Skill,
				SkillCounts[Skill],
			]);
		...
		function calculateSkillPercentages(
			moduleBasedSkillCounts,
			moduleBasedTotalSkillCounts,
		) {
			...
			for (let i = 0; i < moduleBasedTotalSkillCounts?.length; i++) {
				...
				for (const Skill in moduleTotalSkillCount.SkillCounts) {
					if (
						moduleTotalSkillCount.SkillCounts.hasOwnProperty(Skill)
					) {
						const totalSkillCount =
							moduleTotalSkillCount.SkillCounts[Skill];
						const SkillCount = moduleBasedSkillCounts[i]?.find(
							(entry) => entry[0] === Skill,
						);
		...
		const moduleBasedSkillPercentages = calculateSkillPercentages(
			moduleBasedSkillCounts,
			moduleBasedTotalSkillCounts,
		);
	...
	// Usage
	const moduleBasedSkillPercentages = calculateModuleBasedSkillPercentages(
		moduleBasedWrongSkills,
		exam,
	);
	...
	function calculateModuleScores(moduleBasedAllQuestions) {
		...
		const moduleScores = moduleBasedAllQuestions.map((module) => {
			...
			const correctQuestions = module.questions.filter(
	...
	useEffect(() => {
		if (id) {
			dispatch(getUserExamDetails(id));
		}
	}, [dispatch, id]);
				...
				{isLoading ? (
					...
					<Fragment>
						{!exam ? (
							...
							<div className="px-4 pb-96 bg-white text-blue-gray-900 md:px-24 lg:px-32 xl:px-48 py-8 flex flex-col items-center">
																	...
																	if (
																		...
																		return selectedQuestion
																			...
																			<img
																				...
																				alt={
																					optionDetails?.public_id
																				}
																				className="w-auto h-12 inline"
																			/>
																		) : (
																			<span
																				key={
